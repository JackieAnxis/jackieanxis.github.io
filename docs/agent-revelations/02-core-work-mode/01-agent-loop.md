...
> 太长不看:
>
> **当前的 Agent 的本质是一个无限循环**——把"答完即走"变成"持续思考→行动→观察"，让模型在迭代中消化真实任务的不确定性。
>
> 循环四步用"封存快照、新历史追加"的方式让每轮可独立回退，单步出错只影响当前轮，不会污染全局上下文。
>
> 三种终止条件（任务完成、达到上限、出错中断）给无限循环和资源耗尽兜底：模型可能误判为未完成、token 撞到上限、用户中断或不可恢复错误时都该退出。
