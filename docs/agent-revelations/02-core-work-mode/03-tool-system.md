...
> 太长不看:
>
> **工具系统贯穿 Fail-Closed 安全哲学**——默认拒绝、deny 永远优先于 allow、安全检查不可绕过。
>
> 三层过滤管线按顺序独立判断：模式（白名单）→ 规则（黑名单）→ 状态（自检查），一层失守下层仍能拦截——这就是纵深防御的价值。
>
> 其中，ToolSearch 把工具描述从初始上下文剥离改为按需加载，不只是为了省 token，更是为了让模型看不到的工具完全没有被误调用的可能，最小暴露本身就是安全策略。
>
> **无法证明安全就默认不信任。**
...
**延伸阅读**：[MCP 协议：AI 时代的 “USB-C 接口”](../03-peripheral-components/model-context-protocol.md)