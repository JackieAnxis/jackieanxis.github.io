...
> 太长不看:
>
> **权限管线是 Agent 的护栏，核心是纵深防御**——六步独立检查：格式（是否用对了）→ 规则（是否可以用）→ 工具（自定义检查）→ 安全路径（拒绝写入敏感路径）→ 模式（是否可以放行）→ 人工窗口（最后由用户兜底），一步否决全局否决。
>
> `deny > ask > allow` 是优先级铁律，allow 规则永远不能越过 ask 要求人工确认，安全底线不可被规则覆盖。
...
1. **快速阶段**：模型快速给出"放行/阻止"，只分配极少 token（64 个）。提示词刻意偏向"宁可错杀"——明显安全的命令快速放行省 token，可疑命令先标为"阻止"。如果说"放行"，直接通过。
2. **深度阶段**：快速阶段说"阻止"时才触发。给模型更多 token（4096 个）展开完整思考链，对快速阶段的"错杀"做二次复核——如果重新审视后认为安全，可以翻案放行。
