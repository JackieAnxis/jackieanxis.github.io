...
> 太长不看:
> **计划与任务把模糊目标显式化**——拆解让步骤可执行、依赖让顺序清晰、看板让进度可见
>
> 核心内容：复杂任务先进入 Plan Mode 只读思考（搜索代码、阅读文件、设计方案但不修改），让方向在零成本阶段就确认下来——避免动手后才发现错误。
>
> 任务卡显式声明："做什么 + 为什么做 + 前置依赖是什么"，让进度可追踪、压缩后仍可恢复，任务列表本身就是"工作锚点"。三层验证（自动化 → 功能性 → 目标对照）防止"做完了但方向错了"。
