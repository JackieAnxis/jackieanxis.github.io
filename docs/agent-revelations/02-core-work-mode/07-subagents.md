...
> 太长不看:
>
> **Subagent 的核心价值是隔离与专业化**——独立上下文窗口让子任务不污染主对话，不同子 Agent 可配不同模型/工具/权限。
>
> 每个 Subagent 拥有独立对话历史和 token 预算，主 Agent 只接收最终结果——复杂任务的研究/搜索不会撑爆主对话。
>
> Subagent 可指定不同模型节省成本，简单任务不需要最强模型，专业化分工降低开销。代价是 token 成本线性增长。
