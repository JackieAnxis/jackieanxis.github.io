...
**为什么 CLAUDE.md 不放在系统提示词中？** CLAUDE.md（含 `@include` 引用链）可能是上下文里体量最大的部分之一，而大模型服务侧对系统提示词的缓存有窗口限制，这就是为什么 `CLAUDE.md` 不会被放在系统提示词中。而用户消息没有这个约束——体积大了触发压缩即可，压缩后从磁盘重读再注入。所以不是缓存策略的问题，纯粹是体积：不要把最大的东西放进对体积最敏感的通道。
