...
> 太长不看:
>
> **协调器模式是"极端分工"**——剥夺主 Agent 全部动手能力，迫使它只做综合与编排。
>
> 协调器的工具集被截肢为 4 个（Agent/SendMessage/TaskStop/StructuredOutput）——剥夺 Read/Write/Edit 能力是最强的行为约束，防止协调器忍不住自己动手。
>
> Worker 共享研究发现（通过写入文件系统来缓解上下文隔离带来的问题）。与 Fork 模式"复制上下文"相反，适合强顺序、强审计的复杂任务。
>
> **剥夺能力是最强的行为约束。**
