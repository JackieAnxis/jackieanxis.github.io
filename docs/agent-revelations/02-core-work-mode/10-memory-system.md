...
> 太长不看:
>
> **记忆系统是 Agent 自己做的笔记，与 CLAUDE.md 互补**——CLAUDE.md 是静态规矩，记忆是跨会话积累的经验。
>
> 每轮对话从全部记忆文件中检索最相关的 5 条注入上下文——扫描目录解析 frontmatter，按时间倒序选 200 个候选，平铺检索而非全量拼接避免上下文爆炸。
>
> 强制记录 **Why** 和 **How to apply** 而非 What，知道为什么才能在未来判断边界情况，单纯记 What 会让记忆沦为纠错清单。Subagent 记忆作用域独立于主 Agent。
