...
> 太长不看:
>
> **Agent Teams 让多个 Agent 持续在线、互相通信**——核心是"水平通信"，与协调器模式的星型单线汇报相反。
>
> Lead 不替换系统提示词只通过 TeamCreate 工具的 description 获得工作流指导——增加协调层而非限制自由度，Lead 想亲力亲为时随时可以动手。
>
> 队友通过文件收件箱通信而非内存队列（发送即往对方文件追加 JSON 记录，加锁保证原子性）——Split-Pane 模式下队友是独立进程，文件系统是天然共享层。权限冒泡让队友没有自己的终端（500ms 轮询 Lead 收件箱），避免用户在多个 teammate 之间来回切换。代价是 token 线性增长。
